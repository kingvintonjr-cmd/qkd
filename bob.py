...
def main():
    ...
    with CQCConnection("Bob") as Bob:
        ...
        alice_bases = list(Bob.recvClassical())
        ...
        alice_reveal_values = list(Bob.recvClassical())
        ...
        # SimulaQron expects integers/bytes, so scale QBER for transport
        Bob.sendClassical("Alice", [int(qber * 100), decision])

        final_key = sifted_key[num_reveal:]

        print("\n--- Bob Results ---")
        print(f"Measurement Results (first 10): {results[:10]}...")
